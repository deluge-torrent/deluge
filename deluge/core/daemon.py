...
def check_running_daemon(pid_file):
    ...
    of the daemon using the same pid file"""
    if os.path.isfile(pid_file):
    # Get the PID and the port
    # of the supposedly running daemon
        ...
        def process_running(pid):
            ...
            # We can just use os.kill on UNIX \
            # to test if the process is running
                try:
                    os.kill(pid, 0)
                except OSError:
                    return False
        ...
        # Ensure it's a deluged process by trying \
        # to open a socket to it's port.
            import socket
            ...
            try:
                ...
                # This is a deluged!
                _socket.close()
                raise DaemonRunningError("Daemon already running")
...
class Daemon(object):
    def __init__(self, options=None, args=None, classic=False):
        ...
        # so just have it call the shutdown method.
        ...
        if windows_check():
            def win_handler(ctrl_type):
                ...
                or ctrl_type == CTRL_SHUTDOWN_EVENT:
                    self._shutdown()
                    return 1
            SetConsoleCtrlHandler(win_handler)
        listen_interface = None
        if options and options.listen_interface \
        and is_ip(options.listen_interface):
            listen_interface = options.listen_interface
        # Start the core as a thread \
        # and join it until it's done
        ...
        if not classic:
            ...
            # Create pid file to track if deluged \
            # is running, also includes the port number.
            pid = os.getpid()
    ...
    def _shutdown(self, *args, **kwargs):
        log.info("Deluge daemon shutting down, \
        waiting for components to shutdown...")
